...
在配置对话框中，鼠标滚轮划过 `QComboBox` / `QSpinBox` 时会意外切换选项值。添加 `QScrollArea` 后这个问题更加突出——用户期望滚轮用于滚动页面，但被控件截获。

### 方案

在 `QApplication` 上安装全局事件过滤器，拦截所有 `QComboBox` 和 `QAbstractSpinBox`（涵盖 `QSpinBox`、`QDoubleSpinBox`）的 `QEvent::Wheel` 事件。
...
```
鼠标滚轮 → QComboBox / QSpinBox（被拦截）
              │
              ├─ return true（阻止控件自身响应滚轮，值不变）
              │
              └─ sendEvent → 父级 QAbstractScrollArea.viewport()
                                │
                                └─ 页面正常滚动 ✅
```

若控件不在任何 `QScrollArea` 内，则事件被丢弃（无影响）。

### 覆盖范围

| 控件类型 | 基类 | 典型示例 |
|---|---|---|
| 下拉框 | `QComboBox` | 协议选择、编码器选择、主题选择 |
| 整数数字框 | `QSpinBox` | 帧率、码率、字号 |
| 浮点数字框 | `QDoubleSpinBox` | — |

### 注意事项

- `QComboBox` 下拉列表 **展开时**，滚轮事件目标是 popup 的 `QAbstractItemView`，不会被拦截，列表内仍可正常滚动
- 该过滤器覆盖 **整个应用**，无需逐个控件设置
